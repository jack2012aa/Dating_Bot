...
def insert_or_delete_tag(event: PostbackEvent, tag: str):
    ...
    exist = models.exchange_book.has_editting_tag(event.source.user_id, tag)
    if exist:
        models.exchange_book.delete_editting_tag(event.source.user_id, tag)
        line_bot_api.reply_message(event.reply_token, [TextSendMessage(text = text_dict["Delete tag"].format(tag = tag))])
    else:
        models.exchange_book.insert_editting_tag(event.source.user_id, tag)
